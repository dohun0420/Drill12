...
class Zombie:
    ...
    def build_behavior_tree(self):
        ...
        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby)
        c2 = Condition('소년의 공보다 많거나 같은가?', self.should_chase_or_flee)
        a3 = Action('소년에게 접근', self.move_to_boy)
        root = chase_boy = Sequence('소년을 추적', c1, c2, a3)

        c3 = Condition('소년의 공보다 적은가?', lambda: not self.should_chase_or_flee())
        a4 = Action('소년으로부터 도망', self.flee_from_boy)
        root = flee_from_boy = Sequence('소년으로부터 도망', c1, c3, a4)

        root = chase_or_flee_or_wander = Selector('추적 또는 도망 또는 배회', chase_boy, flee_from_boy, wander)

        self.bt = BehaviorTree(chase_or_flee_or_wander)